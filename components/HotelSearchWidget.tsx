  ...
  return (
    <div className="flex-1 flex flex-col justify-between animate-in fade-in duration-300 h-full" ref={widgetRef}>
      ...
      <div className="space-y-4 mb-6 relative">
        ...
        {/* Destination Input */}
        <div 
          className={`bg-[#1e293b] p-2.5 rounded border transition relative cursor-pointer ${activePopup === 'dest' ? 'border-[#ff6b00]' : 'border-gray-700'}`}
        ...
        >
          ...
          <div className="flex justify-between items-center mt-0.5">
            <input type="text" value={destination} readOnly className="w-full bg-transparent text-white font-bold text-xs outline-none cursor-pointer pointer-events-none truncate" />
            <MapPin size={14} className="text-gray-400 shrink-0 ml-1" />
          </div>
          ...
          {activePopup === "dest" && (
            <div className="absolute top-full left-0 mt-2 w-full bg-[#1e293b] border border-gray-600 rounded-xl shadow-2xl z-[60] p-3 animate-in fade-in zoom-in-95" onClick={e => e.stopPropagation()}>
              <div className="flex items-center space-x-2 border-b border-gray-600 pb-2 mb-2">
                <Search size={16} className="text-gray-400" />
                <input 
                  type="text" 
                  placeholder="Where are you going?" 
                  className="w-full bg-transparent text-white outline-none font-semibold text-xs placeholder-gray-500" 
                  autoFocus 
                />
              </div>
              <h4 className="text-white font-bold text-[10px] tracking-wide mb-2 px-1">POPULAR CITIES</h4>
              <div className="max-h-[180px] overflow-y-auto hide-scrollbar">
                {popularCities.map((city, idx) => (
                  <button 
                    key={idx}
                    onClick={() => { setDestination(city); setActivePopup(null); }}
                    className="w-full text-left px-2 py-2 hover:bg-[#111827] hover:text-[#ff6b00] rounded-lg text-xs font-semibold text-gray-300 transition flex items-center"
                  >
                    <MapPin size={12} className="mr-2 text-gray-500" /> {city}
                  </button>
                ))}
              </div>
            </div>
        ...
        {/* Check-in / Check-out Mobile optimized Horizontal Grid */}
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            <div className="overflow-hidden">
              ...
              <input type="text" value={formatDisplayDate(checkIn) || "Select"} readOnly className={`w-full bg-transparent font-bold text-xs outline-none mt-0.5 cursor-pointer pointer-events-none ${!checkIn ? 'text-gray-500' : 'text-white'}`} />
            </div>
          ...
          >
            <div className="overflow-hidden">
              ...
              <input 
                ...
                placeholder="Select"
                readOnly 
                className={`w-full bg-transparent font-bold text-xs outline-none mt-0.5 cursor-pointer pointer-events-none ${!checkOut ? 'text-gray-500' : 'text-white'}`} 
              />
            </div>
            <CalendarIcon size={14} className="text-gray-400 ml-1 shrink-0"/>
        ...
        {/* Guests & Rooms */}
        <div 
          className={`bg-[#1e293b] p-2.5 rounded border flex justify-between items-center transition relative cursor-pointer ${activePopup === 'guests' ? 'border-[#ff6b00]' : 'border-gray-700'}`}
          onClick={() => setActivePopup(activePopup === 'guests' ? null : 'guests')}
        >
          <div className="overflow-hidden">
            <p className="text-[10px] text-gray-400 uppercase font-semibold">Rooms & Guests</p>
            <input type="text" value={`${guestState.rooms} Rm, ${totalGuests} Pax`} readOnly className="w-full bg-transparent text-white font-bold text-xs outline-none mt-0.5 cursor-pointer pointer-events-none" />
          </div>
          <Users size={16} className="text-gray-400 shrink-0 ml-1"/>

          {activePopup === "guests" && (
            <div className="absolute bottom-full right-0 mb-2 w-full bg-[#1e293b] border border-gray-600 rounded-xl shadow-2xl z-[60] p-4 animate-in fade-in zoom-in-95 cursor-default" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center mb-3 pb-2 border-b border-gray-700">
                <h3 className="font-bold text-white text-xs uppercase tracking-wide">Allocation</h3>
                <button onClick={() => setActivePopup(null)} className="text-gray-400 hover:text-white"><X size={14}/></button>
              </div>
              
              {(['rooms', 'adults', 'children'] as const).map((field) => (
                <div key={field} className="flex justify-between items-center py-1.5">
                  <div>
                    <h4 className="font-bold text-gray-200 text-xs capitalize">{field}</h4>
                    <p className="text-[9px] text-gray-500">{field === 'rooms' ? 'Max 10' : field === 'adults' ? 'Ages 13+' : 'Ages 0-12'}</p>
                  </div>
                  <div className="flex items-center space-x-2">
                    <button onClick={() => updateGuests(field, -1)} className="w-6 h-6 rounded border border-gray-600 flex items-center justify-center text-gray-300 hover:bg-gray-700 transition"><Minus size={12}/></button>
                    <span className="font-bold w-4 text-center text-xs text-white">{guestState[field]}</span>
                    <button onClick={() => updateGuests(field, 1)} className="w-6 h-6 rounded border border-gray-600 flex items-center justify-center text-gray-300 hover:bg-gray-700 transition"><Plus size={12}/></button>
                  </div>
                </div>
              ))}
            </div>
      ...
      >
        <Building size={18} className={isSearching ? "animate-pulse" : ""} /> 
