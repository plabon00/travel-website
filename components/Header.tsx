...
export default function Header() {
  ...
  "
      >
        ...
        <div className="flex items-center mr-4 z-50 shrink-0">
          <Link
            href="/"
            className="text-xl lg:text-3xl font-extrabold tracking-tight leading-none"
          >
            SHARK <br />
        ...
        <nav className="hidden md:flex flex-1 justify-center items-center space-x-1 lg:space-x-2 text-[12px] lg:text-sm font-semibold">
          <Link href="/" className={navItemClass("/")}>
            ...
            <Home
              size={16}
              className="mr-1.5 lg:mr-2 lg:w-[18px] lg:h-[18px]"
            />
            <span>Home</span>
          </Link>
          ...
          <div className="relative group py-2 shrink-0">
            <div className={navItemClass("/packages")}>
              ...
              <ChevronDown
                size={14}
                className="ml-1.5 group-hover:rotate-180 transition-transform"
              />
            </div>
  ...
  "
            >
              {" "}
              <Link
                href="/packages/womens-exclusive"
                className="block px-4 py-3 hover:bg-[#ff6b00] transition-colors"
              >
                Women's Exclusive Escapes
              </Link>
              <Link
                href="/packages/sacred-pilgrimages"
                className="block px-4 py-3 hover:bg-[#ff6b00] transition-colors"
              >
                Hajj & Umrah
              </Link>
              <Link
                href="/packages/group-adventures"
                className="block px-4 py-3 hover:bg-[#ff6b00] transition-colors"
              >
                Group Adventures
              </Link>
              <Link
                href="/packages/corporate-retreats"
                className="block px-4 py-3 hover:bg-[#ff6b00] transition-colors"
              >
        ...
        {isMobileMenuOpen && (
          <div className="absolute top-[100%] left-0 w-full bg-[#0b0f19] border-t border-gray-800 shadow-2xl md:hidden flex flex-col p-6 space-y-4 font-semibold z-40">
            <Link
              ...
              className={`flex items-center p-3 rounded-xl ${isActive("/") ? "bg-white/10 text-[#ff6b00]" : "text-gray-200"}`}
            >
            ...
            <Link
              href="/flights"
              onClick={() => setIsMobileMenuOpen(false)}
              className={`flex items-center p-3 rounded-xl ${isActive("/flights") ? "bg-white/10 text-[#ff6b00]" : "text-gray-200"}`}
            >
            ...
            <Link
              href="/hotels"
              onClick={() => setIsMobileMenuOpen(false)}
              className={`flex items-center p-3 rounded-xl ${isActive("/hotels") ? "bg-white/10 text-[#ff6b00]" : "text-gray-200"}`}
            >
            ...
            <Link
              href="/hajj-umrah"
              onClick={() => setIsMobileMenuOpen(false)}
              className={`flex items-center p-3 rounded-xl ${isActive("/hajj-umrah") ? "bg-white/10 text-[#ff6b00]" : "text-gray-200"}`}
            >
            ...
            <Link
              href="/visa"
              onClick={() => setIsMobileMenuOpen(false)}
              className={`flex items-center p-3 rounded-xl ${isActive("/visa") ? "bg-white/10 text-[#ff6b00]" : "text-gray-200"}`}
            >
            ...
            <Link
              href="/packages"
              onClick={() => setIsMobileMenuOpen(false)}
              className={`flex items-center p-3 rounded-xl ${isActive("/packages") ? "bg-white/10 text-[#ff6b00]" : "text-gray-200"}`}
            >
